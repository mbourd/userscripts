...
(async function () {
  ...
  function executeIntervalCheck() {
    interval = window[_setInterval](async () => {
      if (isWatching) {
        const skipNext = () => window[_document][_querySelector](_strSelector002) || window[_document][_querySelector](_strSelector009);
        // const skipRecap = ()=>window[_document][_querySelector](_strSelector007);
        const next = () => window[_document][_querySelector](_strSelector003);
        const playPause = () => window[_document][_querySelector](_strSelector004);
        const video = () => window[_document][_querySelectorAll](string011)[_number0];
        const timeIndicator = () => window[_document][_querySelector](_strSelector008);
        const adTimeIndicator = () => window[_document][_querySelector](_strSelector012);
      ...
      window[_console][_log]({ t, c: "c:" + video()[_currentTime], r: "r:" + (video()[_duration] - video()[_currentTime]) });
      return t;
    })());
  }
})();
